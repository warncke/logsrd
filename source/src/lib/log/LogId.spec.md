...
- `#base64: string | null` — cached Base64URL string
- `#logDirPrefix: string | null` — cached directory prefix

### Dependency graph

```
LogId ──► crypto (mz/crypto)
```
...
sequenceDiagram
    ...
    alt #logDirPrefix is null
        Instance->>Instance: hex of byte[0] + "/" + hex of byte[1]
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    ...
    { label: "toJSON() / u8s()", detail: "Serialization for transport/storage" },
  ];
...
|---|---|---|---|
| 1 | `newRandom` returns LogId with 16-byte Uint8Array | Unit | `instance.logId.byteLength === 16` |
| 2 | `base64()` round-trips | Unit | `LogId.newFromBase64(id.base64()).base64() === id.base64()` |
| 3 | `base64()` is cached | Unit | Second call returns same string without recomputation |
| 4 | `logDirPrefix` format | Unit | Matches `"XX/XX"` where XX are lowercase hex of first two bytes |
| 5 | `logDirPrefix` is cached | Unit | Second call returns same string without recomputation |
| 6 | `toJSON` returns base64 | Unit | `id.toJSON() === id.base64()` |
| 7 | `u8s()` returns `[logId]` | Unit | Array with single element equal to `this.logId` |
| 8 | `newFromBase64` with invalid string | Unit | Throws or returns incorrect-length Uint8Array (verify 16 bytes) |
| 9 | Constructor with pre-computed base64 uses it | Unit | Passing base64 skips encoding on first `base64()` call |
| 10 | `byteLength()` returns 16 | Unit | For any valid LogId |
...
| Test Spec | Path |
|---|---|
| LogId.test.spec.md | `source/src/lib/log/LogId.test.spec.md` |
