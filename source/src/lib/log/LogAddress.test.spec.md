...
- At least 22 characters required
...
|---|---|---|---|
| 1 | Create with logIdBase64 only | Unit | `logIdBase64` set, `host` and `config` null |
| 2 | Create with logIdBase64 and host | Unit | `host.master` correctly set from LogHost |
| 3 | setConfig replaces config hosts | Unit | `config` array has correct length and master |
| 4 | setHost replaces host | Unit | `host.master` and `host.replicas` updated |
| 5 | toString with only logId | Unit | Returns just the base64 string |
| 6 | toString with host | Unit | Returns `"b64id;master,replica1"` |
| 7 | toString with host and config | Unit | Returns `"b64id;host;config"` triple-section |
| 8 | fromString parses full address | Unit | All three sections correctly extracted |
| 9 | fromString with only logId | Unit | `host=null`, `config=null` |
| 10 | Invalid short address throws | Unit | String < 22 characters throws |
| 11 | Multiple config hosts parsed | Unit | Extra sections populate `config` array correctly |
...
| Source Spec | Path |
|---|---|
| LogAddress.spec.md | `source/src/lib/log/LogAddress.spec.md` |
