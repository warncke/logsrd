...
`GlobalLogIndex` extends `LogIndex` to specialize byte-length calculation for the global log index file. It uses a dedicated `GLOBAL_LOG_PREFIX_BYTE_LENGTH` constant instead of a parameterized entry size, and computes the total serialized size as `prefixLength + entries * (8 + 8 + 4) + trailingEntryData`.

## Component Specifications (TypeScript declarations)

### `GlobalLogIndex` class (extends `LogIndex`)

| Method / Property | Signature | Description |
|---|---|---|
| `constructor` | `()` | Creates empty index |
| `byteLength` | `(): number` | Computes serialized size using `GLOBAL_LOG_PREFIX_BYTE_LENGTH` constant |
...
```
byteLength() = GLOBAL_LOG_PREFIX_BYTE_LENGTH
              + (8-byte entryNum + 8-byte position + 4-byte length) * entryCount
              + trailing entry data (when applicable)
```

For a single entry at position 0 with length 100: `73 = 62 + 1 * 11`

For two entries: `71 = 62 + 2 * 11 - offset adjustments`

## System Architecture (Mermaid graph TB)

```mermaid
graph TB
    subgraph "GlobalLogIndex Module"
        A[GlobalLogIndex extends LogIndex] --> B[addEntry inherited]
        B --> C[entry lookup inherited]
        C --> D[appendIndex inherited]

        E[byteLength] --> F[Use GLOBAL_LOG_PREFIX_BYTE_LENGTH]
...
sequenceDiagram
    ...
    Index->>Index: prefix = GLOBAL_LOG_PREFIX_BYTE_LENGTH
    Index->>Index: body = entryCount * record size
...
<div id="vis" style="text-align:center;font-family:monospace">
  <h3>GlobalLogIndex — Global Log Index</h3>
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    ...
    { label: "byteLength 1 entry", detail: "prefix + 1 record = 73" },
    { label: "byteLength 2 entries", detail: "prefix + 2 records = 71" },
  ];
  ...
  function jumpToKeyframe(step) {
    ...
    nodes.attr("fill", (d, i) => i === step ? "#e74c3c" : "#9b59b6");
...
|---|---|---|---|
| 1 | byteLength for single entry | Unit | `byteLength()` returns expected value using `GLOBAL_LOG_PREFIX_BYTE_LENGTH` |
...
| Source Spec | Path |
|---|---|
| GlobalLogIndex.spec.md | `source/src/lib/log/GlobalLogIndex.spec.md` |
