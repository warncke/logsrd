...
`LogLogIndex` extends `LogIndex` with an overridden `byteLength()` method that subtracts `LOG_LOG_PREFIX_BYTE_LENGTH` instead of a caller-supplied prefix length. It reuses all other `LogIndex` behavior (entry triplet storage, config tracking, merging, querying). It mirrors `GlobalLogIndex` in structure but targets a different prefix constant.
...
|---|---|---|
| `byteLength()` | `(): number` | Overrides `LogIndex.byteLength`; sums `(length - LOG_LOG_PREFIX_BYTE_LENGTH)` over all entries in `en` |
...
```
LogLogIndex ──► LogIndex
LogLogIndex ──► LOG_LOG_PREFIX_BYTE_LENGTH (from globals)
```
...
```mermaid
graph TB
    subgraph "LogLogIndex"
        A[byteLength override] --> B[Iterate en triplets]
        B --> C[Sum (length - LOG_LOG_PREFIX_BYTE_LENGTH)]
    end
    ...
    subgraph "Globals"
        H[LOG_LOG_PREFIX_BYTE_LENGTH] --> A
    end

    LogLogIndex -- inherits --> D
    LogLogIndex -- inherits --> F
    LogLogIndex -- inherits --> G
```
...
sequenceDiagram
    ...
    Index->>Index: for each triplet: sum(length - LOG_LOG_PREFIX_BYTE_LENGTH)
    Note over Index: Uses LOG_LOG prefix constant
...
<div id="vis" style="text-align:center;font-family:monospace">
  <h3>LogLogIndex — byteLength Override</h3>
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    ...
    { label: "byteLength()", detail: "Sum((length - LOG_LOG_PREFIX_BYTE_LENGTH) ...)" },
  ];
...
| 2 | `byteLength()` uses `LOG_LOG_PREFIX_BYTE_LENGTH` | Unit | Sum computed with correct constant, not parameter |
...
| 5 | Inherited `entry` lookup | Unit | Arithmetic index offset works for log-log index |
...
| Test Spec | Path |
|---|---|
| LogLogIndex.test.spec.md | `source/src/lib/log/LogLogIndex.test.spec.md` |
