...
`IOQueue` provides a simple FIFO queue for I/O operations, separating reads and writes into separate lists. It supports `enqueue`, `getReady` (returns all pending ops), and `drain` (clears all ops). The queue exposes whether operations are pending via `opPending()`.
...
graph TB
    subgraph "IOQueue"
        Q[IOQueue]
        Q --> READS[readOps: IOOperation[]]
        Q --> WRITES[writeOps: IOOperation[]]
    end
...
sequenceDiagram
    ...
    App->>OP: new(WRITE)
    App->>Q: enqueue(op)
    Q->>Q: add to writeOps or readOps
    App->>Q: opPending()
    Q-->>App: true

    App->>Q: getReady()
    Q->>Q: return all [reads, writes]
    Q->>Q: clear internal lists
    Q-->>App: [readOps, writeOps]
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    { label: "Queue created empty", phase: "init", desc: "opPending() = false" },
    { label: "Enqueue write operation", phase: "enqueue", desc: "Operation added to write queue" },
    { label: "Enqueue read operation", phase: "enqueue", desc: "Operation added to read queue" },
    { label: "getReady returns ops", phase: "ready", desc: "Returns [reads, writes], clears queue" },
    { label: "Drain all operations", phase: "drain", desc: "Returns all ops, queue empty" },
  ];
  const ANIMATION_VERIFICATION = [
    "opPending() is false for empty queue",
    "Enqueue write makes opPending() true",
    "Enqueue read makes opPending() true",
    "getReady returns separate read and write arrays",
    "getReady clears the queue (opPending = false after)",
    "drain returns all operations and clears queue",
    "getReady on empty queue returns empty arrays",
  ];

  const LEGEND = [
    { label: "Init", color: "#b2df8a" },
    { label: "Enqueue", color: "#fdbf6f" },
    { label: "Ready", color: "#a6cee3" },
    { label: "Drain", color: "#f781bf" },
  ];
...
| # | Test | Expected |
|---|------|----------|
| 1 | New queue is empty | `opPending()` returns `false` |
| 2 | Enqueue write operation | `opPending()` returns `true` |
| 3 | Enqueue read operation | `opPending()` returns `true` |
| 4 | `getReady()` returns ops and clears queue | Returns `[reads, writes]` with correct lengths |
| 5 | `drain()` returns all ops | Returns `[reads, writes]`, `opPending()` = `false` |
| 6 | `getReady()` on empty queue returns empty arrays | Both arrays have length 0 |
...
| Source Spec | Path |
|---|---|
| IOQueue.spec.md | `source/src/lib/persist/io/IOQueue.spec.md` |
