...
`ReadRangeIOOperation` is a subclass of `IOOperation` for reading raw byte ranges from a log file. It either specifies an array of file offsets to read or is a null read (no specific offsets). It stores the resulting buffers and bytes read count.
...
graph TB
    subgraph "ReadRangeIOOperation"
        ...
        RRIO -->|extends| IO[IOOperation]
        RRIO --> READS[reads: number[] | null]
        RRIO --> BUFS[buffers: Uint8Array[]]
        RRIO --> BR[bytesRead: number]
    end
```
...
sequenceDiagram
    ...
    participant RRIO as ReadRangeIOOperation

    App->>RRIO: new([0, 100, 200])
    RRIO-->>App: reads=[0,100,200], buffers=[], bytesRead=0

    App->>RRIO: new()
    RRIO-->>App: reads=null, buffers=[], bytesRead=0
```
...
<head>
<meta charset="utf-8">
<title>ReadRangeIOOperation Animation</title>
<style>
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    { label: "Create with reads", phase: "create", desc: "reads=[0,100,200], buffers=[]" },
    { label: "Create null read", phase: "create", desc: "reads=null, buffers=[]" },
  ];
  const ANIMATION_VERIFICATION = [
    "Constructor sets reads array from parameter",
    "buffers is initially empty array",
    "bytesRead is initially 0",
    "reads can be null when not provided",
  ];

  const LEGEND = [
    { label: "Create", color: "#b2df8a" },
    { label: "Null", color: "#a6cee3" },
  ];
...
| # | Test | Expected |
|---|------|----------|
| 1 | Create with `reads = [0, 100, 200]` | `reads` = `[0, 100, 200]`, `buffers` = `[]`, `bytesRead` = 0 |
| 2 | Create with no arguments | `reads` = null |
...
| Source Spec | Path |
|---|---|
| ReadRangeIOOperation.spec.md | `source/src/lib/persist/io/ReadRangeIOOperation.spec.md` |
