...
`ReadEntryIOOperation` is a subclass of `IOOperation` for reading a single entry from a log. It wraps a `LogIndex` for metadata lookup, an entry number to identify the entry, and stores the result (`entry`) plus bytes read count.
...
graph TB
    subgraph "ReadEntryIOOperation"
        RIO[ReadEntryIOOperation]
        RIO -->|extends| IO[IOOperation]
        RIO --> IDX[index: LogIndex]
        RIO --> EN[entryNum: number]
        RIO --> ENT[entry: LogEntry | null]
        RIO --> BR[bytesRead: number]
    end
```
...
sequenceDiagram
    ...
    participant RIO as ReadEntryIOOperation
    participant Index as LogIndex

    App->>Index: new()
    App->>RIO: new(logId, index, 5)
    RIO-->>App: index set, entryNum=5, entry=null, bytesRead=0
```
...
<head>
<meta charset="utf-8">
<title>ReadEntryIOOperation Animation</title>
<style>
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    { label: "Create operation", phase: "create", desc: "Store logId, index, entryNum" },
    { label: "Initial state", phase: "state", desc: "entry=null, bytesRead=0" },
  ];
  const ANIMATION_VERIFICATION = [
    "Constructor sets index, entryNum",
    "entry is initially null",
...
| # | Test | Expected |
|---|------|----------|
| 1 | Create with `logId`, `index`, and `entryNum=5` | `index` set, `entryNum` = 5, `entry` = null, `bytesRead` = 0 |
...
| Source Spec | Path |
|---|---|
| ReadEntryIOOperation.spec.md | `source/src/lib/persist/io/ReadEntryIOOperation.spec.md` |
