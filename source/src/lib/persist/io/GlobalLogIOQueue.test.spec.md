...
`GlobalLogIOQueue` is a multi-queue manager that maintains a global queue and per-log queues keyed by `LogId`. It supports enqueuing operations on specific log queues, querying queue state, and retrieving ready operations sorted by their order. Per-log queues can be created and deleted dynamically.
...
class GlobalLogIOQueue {
  ...
  getReady(): [IOOperation[], IOOperation[]]
...
graph TB
    subgraph "GlobalLogIOQueue"
        ...
        GLQ --> GQ[globalQueue: IOQueue]
        GLQ --> LQM[logQueues: Map<LogId, IOQueue>]
    end

    subgraph "Operations"
        GLQ --> ENQ[enqueue]
        GLQ --> READY[getReady]
        GLQ --> GLQGET[getLogQueue]
        GLQ --> GQGET[getGlobalQueue]
        GLQ --> DEL[deleteLogQueue]
        GLQ --> PEND[opPending]
    end
```
...
sequenceDiagram
    ...
    participant GLQ as GlobalLogIOQueue
    participant Q as IOQueue (per-log)

    App->>GLQ: new()
    App->>GLQ: opPending()
    GLQ-->>App: false

    App->>GLQ: enqueue(op) [no logId → global]
    GLQ->>GLQ: add to globalQueue
    App->>GLQ: opPending()
    GLQ-->>App: true

    App->>GLQ: enqueue(op) [with logId]
    GLQ->>GLQ: get or create log queue
    GLQ->>Q: enqueue(op)

    App->>GLQ: getLogQueue(logId)
    GLQ-->>App: IOQueue instance

    App->>GLQ: deleteLogQueue(logId)
    GLQ-->>App: IOQueue (deleted)

    App->>GLQ: getReady()
    GLQ->>GLQ: merge global + all log queues
    GLQ->>GLQ: sort by order
    GLQ-->>App: [reads, writes]
```
...
<head>
<meta charset="utf-8">
<title>GlobalLogIOQueue Animation</title>
<style>
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    { label: "Queue manager created", phase: "init", desc: "Global + per-log queues" },
    { label: "Enqueue global operation", phase: "enqueue", desc: "No logId → global queue" },
    { label: "Enqueue log operation", phase: "enqueue", desc: "With logId → per-log queue" },
    { label: "Get log queue", phase: "query", desc: "Create/get IOQueue for logId" },
    { label: "Delete log queue", phase: "delete", desc: "Remove queue, return it" },
    { label: "Get ready sorted by order", phase: "ready", desc: "Merge + sort all ready ops" },
  ];
  const ANIMATION_VERIFICATION = [
    "opPending() is false on empty queue manager",
    "Enqueue global op makes opPending() true",
    "Enqueue log-specific op makes opPending() true",
    "getLogQueue returns an IOQueue instance",
    "getGlobalQueue returns the global IOQueue",
    "deleteLogQueue removes and returns the queue",
    "deleteLogQueue returns null for non-existent queue",
    "getReady sorts writes by order ascending",
    "opPending() returns false when no queues have ops",
  ];

  const LEGEND = [
    { label: "Init", color: "#b2df8a" },
    { label: "Enqueue", color: "#fdbf6f" },
    { label: "Query", color: "#a6cee3" },
    { label: "Delete", color: "#fb9a99" },
    { label: "Ready", color: "#f781bf" },
  ];
  ...
  svg.append("text")
    ...
    .text("GlobalLogIOQueue Operations");
...
| # | Test | Expected |
|---|------|----------|
| 1 | New queue manager is empty | `opPending()` returns `false` |
| 2 | Enqueue global operation (no logId) | `opPending()` returns `true` |
| 3 | Enqueue log-specific operation (with logId) | `opPending()` returns `true` |
| 4 | `getLogQueue()` returns IOQueue instance | Queue is defined, opPending is false |
| 5 | `getGlobalQueue()` returns global IOQueue | Defined queue instance |
| 6 | `deleteLogQueue()` removes and returns queue | Returns queue, second delete returns null |
| 7 | `getReady()` returns sorted writes | Writes sorted by order ascending |
| 8 | `opPending()` false when log queue has no ops | Even after getLogQueue, opPending is false |
...
| Source Spec | Path |
|---|---|
| GlobalLogIOQueue.spec.md | `source/src/lib/persist/io/GlobalLogIOQueue.spec.md` |
