...
`WriteIOOperation` is a subclass of `IOOperation` specifically for write operations. It wraps a `GlobalLogEntry` to be written and optionally tracks the assigned entry number and bytes written count.
...
class WriteIOOperation extends IOOperation {
  ...
  constructor(entry: GlobalLogEntry, resolve?: ((value: this) => void) | null, entryNum?: number | null)
}
```
...
graph TB
    subgraph "WriteIOOperation"
        ...
        WIO -->|extends| IO[IOOperation]
        WIO --> ENTRY[entry: GlobalLogEntry]
        WIO --> EN[entryNum: number | null]
        WIO --> BW[bytesWritten: number]
    end
```
...
sequenceDiagram
    ...
    participant WIO as WriteIOOperation
    participant GLE as GlobalLogEntry

    App->>GLE: new(...)
    App->>WIO: new(entry)
    WIO-->>App: entry set, entryNum=null, bytesWritten=0

    App->>WIO: new(entry, null, 5)
    WIO-->>App: entryNum=5
```
...
<head>
<meta charset="utf-8">
<title>WriteIOOperation Animation</title>
<style>
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    { label: "Create with entry", phase: "create", desc: "Store GlobalLogEntry reference" },
    { label: "entryNum null initially", phase: "state", desc: "entryNum starts as null" },
    { label: "Create with entryNum", phase: "create", desc: "entryNum set to provided value" },
  ];
  const ANIMATION_VERIFICATION = [
    "Constructor stores the entry reference",
    "entryNum is null when not provided",
    "bytesWritten starts at 0",
    "entryNum can be set via constructor parameter",
  ];

  const LEGEND = [
    { label: "Create", color: "#b2df8a" },
    { label: "State", color: "#a6cee3" },
  ];
...
| # | Test | Expected |
|---|------|----------|
| 1 | Create with `GlobalLogEntry` | `entry` matches, `entryNum` is null, `bytesWritten` = 0 |
| 2 | Create with `entryNum` parameter | `entryNum` = 5 |
...
| Source Spec | Path |
|---|---|
| WriteIOOperation.spec.md | `source/src/lib/persist/io/WriteIOOperation.spec.md` |
