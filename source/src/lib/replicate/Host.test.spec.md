# Host Specification (Replication)

## 1. Overview

`Host` represents a remote replication peer in the replication subsystem. It stores the host name (address:port) and a reference to the `Replicate` instance, providing access to server configuration such as monitor interval, timeout, secret, and local host address.
...
```mermaid
graph TB
    subgraph "Host (Replica Peer)"
        H[Host]
        H --> NAME[host: string - address:port]
        H --> REP[replicate: Replicate instance]
    end

    subgraph "Replicate Server Config"
        REP --> CFG[server.config]
        CFG --> MON[hostMonitorInterval]
        CFG --> TO[replicateTimeout]
        CFG --> SEC[secret]
        CFG --> LOCAL[host - local address]
    end
```
...
sequenceDiagram
    ...
    App->>Repl: create server with config
    App->>H: new(replicate, "127.0.0.1:7001")
    H->>Repl: store reference
    H-->>App: host = "127.0.0.1:7001"
    H-->>App: replicate = replicate
```
...
<head>
<meta charset="utf-8">
<title>Host (Replication) Animation</title>
<style>
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    { label: "Initialize Host", phase: "init", desc: "Store host name and replicate ref" },
    { label: "Access config", phase: "config", desc: "Read replicate.server.config" },
  ];
  const ANIMATION_VERIFICATION = [
    "Constructor sets host property correctly",
    "Constructor sets replicate reference correctly",
  ];

  const LEGEND = [
    { label: "Init", color: "#b2df8a" },
    { label: "Config", color: "#a6cee3" },
  ];
...
| # | Test | Expected |
|---|------|----------|
| 1 | Initialize with host name and replicate ref | `host` = `"127.0.0.1:7001"`, `replicate` = replicate instance |
...
| Source Spec | Path |
|---|---|
| Host.spec.md | `source/src/lib/replicate/Host.spec.md` |
