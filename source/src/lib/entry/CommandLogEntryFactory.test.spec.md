# CommandLogEntryFactory Specification

## 1. Overview

`CommandLogEntryFactory` deserializes `CommandLogEntry` instances from raw `Uint8Array` buffers. It reads the entry type byte (must be `EntryType.COMMAND`), extracts the command name byte, and routes to the appropriate concrete command class (e.g., `CreateLogCommand`, `SetConfigCommand`) via the `COMMAND_CLASS` registry.

## 2. Component Specifications (TypeScript Declarations)

```typescript
class CommandLogEntryFactory {
  static fromU8(buffer: Uint8Array): CommandLogEntry
}
```
...
graph TB
    subgraph "CommandLogEntryFactory"
        ...
        CLEF -->|fromU8| DES[Deserialize]
    end

    subgraph "Dispatch"
        DES --> TYPE[Read EntryType byte]
        TYPE -->|must be 0x04| CNAME[Read commandName byte]
        CNAME --> REG[COMMAND_CLASS registry]
    end

    subgraph "Command Classes"
        REG --> CR[CreateLogCommand]
        REG --> SC[SetConfigCommand]
    end
```
...
sequenceDiagram
    ...
    participant CF as CommandLogEntryFactory
    participant REG as COMMAND_CLASS
    participant CMD as CreateLogCommand or SetConfigCommand

    App->>CF: fromU8(buffer)
    CF->>CF: read byte[0] = EntryType
    alt EntryType !== COMMAND
        CF-->>App: throw "Invalid entryType"
    else valid
        CF->>CF: read byte[1] = commandName
        alt commandName undefined/missing
            CF-->>App: throw "Invalid commandName"
        else valid
            CF->>REG: lookup class by commandName
            REG-->>CF: constructor
            CF->>CMD: new instance with buffer data
            CMD-->>CF: typed command
            CF-->>App: CommandLogEntry instance
        end
    end
```
...
<head>
<meta charset="utf-8">
<title>CommandLogEntryFactory Animation</title>
<style>
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    ...
    { label: "Verify EntryType byte", phase: "validate", desc: "First byte must be COMMAND (0x04)" },
    { label: "Read commandName byte", phase: "parse", desc: "Byte [1] indexes into COMMAND_CLASS" },
    { label: "Lookup in COMMAND_CLASS", phase: "dispatch", desc: "Map commandName to constructor" },
    { label: "Return typed command", phase: "output", desc: "Concrete command instance returned" },
  ];
  const ANIMATION_VERIFICATION = [
    "fromU8 throws 'Invalid entryType' if first byte is not COMMAND",
    "fromU8 throws 'Invalid commandName' if commandName byte is unknown",
    "fromU8 throws 'Invalid commandName' if commandName byte is missing",
    "Parsed CREATE_LOG command is instance of CreateLogCommand",
    "Parsed SET_CONFIG command is instance of SetConfigCommand",
  ];

  const LEGEND = [
    { label: "Validate", color: "#fb9a99" },
    { label: "Parse", color: "#b2df8a" },
    { label: "Dispatch", color: "#fdbf6f" },
  ...
  svg.append("text")
    ...
    .text("CommandLogEntryFactory Dispatch");
...
| 2 | `fromU8()` with invalid command name | Throws `"Invalid commandName"` |
| 3 | `fromU8()` with undefined command name (single byte buffer) | Throws `"Invalid commandName"` |
| 4 | `fromU8()` with `CREATE_LOG` command | Returns instance of `CreateLogCommand` |
| 5 | `fromU8()` with `SET_CONFIG` command | Returns instance of `SetConfigCommand` |
...
| Source Spec | Path |
|---|---|
| CommandLogEntryFactory.spec.md | `source/src/lib/entry/CommandLogEntryFactory.spec.md` |
