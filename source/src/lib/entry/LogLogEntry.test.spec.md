...
`LogLogEntry` wraps a `LogEntry` with an 11-byte binary prefix that includes the entry number (Uint32LE), the nested entry's byte length (Uint16LE), and a CRC32 checksum. It is the envelope for entries persisted within an individual logical log file (as opposed to the global hot log). Unlike `GlobalLogEntry`, it does not carry a `LogId` since the owning log is implicit from the file context.
...
(function() {
  ...
  const ANIMATION_VERIFICATION = [
    ...
    "entryNum at [1..4] as Uint32LE must equal this.entryNum",
    "Length at [5..6] as Uint16LE must equal this.entry.byteLength()",
  ...
  function jumpToKeyframe(idx) {
    ...
    else if (idx === 1) { highlightStart = 1; highlightEnd = 5; }
    else if (idx === 2) { highlightStart = 5; highlightEnd = 7; }
    else if (idx === 3) { highlightStart = 7; highlightEnd = 11; }
    else if (idx === 4) { highlightStart = 0; highlightEnd = 11; }
...
| 6 | `prefixU8()` produces 11-byte `Uint8Array` | `byteLength === 11` |
| 7 | `prefixU8()` is memoized (second call returns same buffer) | Same reference |
| 8 | `u8s()` returns `[prefixU8(), ...entry.u8s()]` | Array length = 3 |
| 9 | `u8()` delegates to `entry.u8()` | Deep equal |
| 10 | `verify()` returns `false` when `crc` is `0` (mismatch) | `false` |
...
| Source Spec | Path |
|---|---|
| LogLogEntry.spec.md | `source/src/lib/entry/LogLogEntry.spec.md` |
