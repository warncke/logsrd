# LogEntryFactory Specification

## 1. Overview

`LogEntryFactory` provides static factory methods for deserializing log entries from raw `Uint8Array` buffers. It routes to the correct entry type via the first byte (entry type) using the `ENTRY_CLASS` registry. It supports full deserialization (`fromU8`), partial buffering (`fromPartialU8`), and validates entry types.

## 2. Component Specifications (TypeScript Declarations)

```typescript
class LogEntryFactory {
  static fromU8(buffer: Uint8Array): LogEntry
  static fromPartialU8(buffer: Uint8Array): { entry?: LogEntry; needBytes?: number; err?: Error }
}
```
...
```mermaid
graph TB
    subgraph "LogEntryFactory"
        LEF[LogEntryFactory]
        LEF -->|fromU8| DES[Deserialize full entry]
        LEF -->|fromPartialU8| PART[Deserialize partial entry]
    end

    subgraph "Entry Type Dispatch"
        LEF --> REG[ENTRY_CLASS registry]
        REG --> GLE[GlobalLogEntry]
        REG --> LLE[LogLogEntry]
        REG --> CLE[CommandLogEntry]
        REG --> BE[BinaryLogEntry]
        REG --> JE[JSONLogEntry]
        REG --> GLC[GlobalLogCheckpoint]
        REG --> LLC[LogLogCheckpoint]
    end

    subgraph "Callers"
        READ[Read Entry IO Operations]
        READ --> LEF
    end
```
...
sequenceDiagram
    ...
    participant REG as ENTRY_CLASS
    participant CMD as CommandLogEntryFactory
    participant GLE as GlobalLogEntryFactory

    App->>LEF: fromU8(buffer)
    LEF->>LEF: read first byte = entryType
    LEF->>REG: lookup class by entryType
    REG-->>LEF: constructor reference
    LEF->>LEF: call class.fromU8(buffer)
    LEF-->>App: LogEntry instance

    App->>LEF: fromPartialU8(buffer)
    LEF->>LEF: check if buffer is empty → needBytes=1
    LEF->>LEF: validate entryType
    LEF-->>App: { entry?, needBytes?, err? }
```
...
<head>
<meta charset="utf-8">
<title>LogEntryFactory Animation</title>
<style>
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    { label: "Receive buffer", phase: "input", desc: "Uint8Array bytes arrive for deserialization" },
    { label: "Read entryType byte", phase: "parse", desc: "Extract first byte as entry type code" },
    { label: "Lookup in ENTRY_CLASS", phase: "dispatch", desc: "Map entryType to constructor via registry" },
    { label: "Call fromU8 on handler", phase: "deserialize", desc: "Delegate to typed factory method" },
    { label: "Return LogEntry instance", phase: "output", desc: "Fully deserialized entry" },
    { label: "Partial buffer: needBytes", phase: "partial", desc: "Empty or incomplete buffer returns needBytes" },
  ];
  const ANIMATION_VERIFICATION = [
    "fromU8 throws 'Invalid entryType' for unknown type byte",
    "fromPartialU8 returns needBytes=1 for empty buffer",
    "fromPartialU8 returns err for invalid non-global/log entry types",
    "fromU8 can parse COMMAND type entry correctly",
    "Deserialized entry matches original structure",
  ];

  const LEGEND = [
    { label: "Buffer bytes", color: "#a6cee3" },
    { label: "Entry Type", color: "#f781bf" },
    { label: "Dispatch", color: "#b2df8a" },
    { label: "Result", color: "#fdbf6f" },
  ];
  ...
  svg.append("text")
    ...
    .text("LogEntryFactory Dispatch Flow");
...
| # | Test | Expected |
|---|------|----------|
| 1 | `fromU8()` with invalid entry type byte | Throws `"Invalid entryType"` |
| 2 | `fromPartialU8()` with empty buffer | Returns `{ needBytes: 1 }` |
| 3 | `fromPartialU8()` with non-global/log entry type | Returns `{ err }` with message containing "Invalid entryType" |
| 4 | `fromU8()` with undefined entryType | Throws `"Invalid entryType"` |
| 5 | `fromU8()` with COMMAND type entry | Returns a deserialized entry |
...
| Source Spec | Path |
|---|---|
| LogEntryFactory.spec.md | `source/src/lib/entry/LogEntryFactory.spec.md` |
