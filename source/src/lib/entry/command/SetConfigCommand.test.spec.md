# SetConfigCommand Specification

## 1. Overview

`SetConfigCommand` is a concrete `CommandLogEntry` that represents a configuration update directive. It deserializes its `commandValueU8` payload as JSON with fields such as `logId`, `type`, `master`, `access`, `authType`, and `stopped`. It behaves similarly to `CreateLogCommand` but uses `CommandName.SET_CONFIG` as its command name byte.

## 2. Component Specifications (TypeScript Declarations)

```typescript
class SetConfigCommand extends CommandLogEntry {
...
```mermaid
graph TB
    subgraph "SetConfigCommand"
        SCC[SetConfigCommand]
        SCC -->|extends| CLE[CommandLogEntry]
    end
...
sequenceDiagram
    ...
    participant SCC as SetConfigCommand

    App->>SCC: new({ value: {logId, type, master, ...} })
    SCC->>SCC: serialize value to JSON bytes
    SCC-->>App: value() returns parsed object

    App->>SCC: new({ commandNameU8, value })
    SCC->>SCC: explicit commandNameU8
    SCC-->>App: value() returns { logId: "test", ... }
```
...
<head>
<meta charset="utf-8">
<title>SetConfigCommand Animation</title>
<style>
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    { label: "Construct with value object", phase: "construct", desc: "Pass config value object" },
    { label: "Serialize to JSON bytes", phase: "serialize", desc: "JSON.stringify config object" },
    { label: "value() returns config", phase: "output", desc: "Access logId property" },
  ];
  const ANIMATION_VERIFICATION = [
    "Constructing with value object returns correct logId via value()",
...
| # | Test | Expected |
|---|------|----------|
| 1 | Construct with `value` object | `value()` returns object with `logId: "test"` |
| 2 | Construct with explicit `commandNameU8` + `value` | `value()` returns object with `logId: "test"` |
...
| Source Spec | Path |
|---|---|
| SetConfigCommand.spec.md | `source/src/lib/entry/command/SetConfigCommand.spec.md` |
