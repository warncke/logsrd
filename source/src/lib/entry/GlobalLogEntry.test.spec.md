...
| 5 | `cksum()` returns a non-zero number | Type is number, value not 0 |
| 6 | `prefixU8()` produces 27-byte `Uint8Array` | `byteLength === 27` |
| 7 | `u8s()` returns `[prefixU8(), ...entry.u8s()]` | Array length = 3 |
| 8 | `verify()` returns `false` when `crc` is `null` | `false` |
| 9 | `verify()` returns `false` when `crc` is `0` (mismatch) | `false` |
| 10 | `prefixU8()` is memoized (second call returns same buffer) | Same reference |
| 11 | `cksum()` is memoized (second call returns same value) | Same value |
| 12 | `u8()` delegates to `entry.u8()` | Deep equal |
| 13 | `verify()` returns `true` when `crc` matches computed `cksum()` | `true` |
...
| Source Spec | Path |
|---|---|
| GlobalLogEntry.spec.md | `source/src/lib/entry/GlobalLogEntry.spec.md` |
