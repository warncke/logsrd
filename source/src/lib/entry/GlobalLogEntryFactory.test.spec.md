# GlobalLogEntryFactory Specification

## 1. Overview

`GlobalLogEntryFactory` provides static methods for deserializing `GlobalLogEntry` instances from raw `Uint8Array` buffers. It handles the 27-byte prefix parsing (entry type, LogId, entry number, length, CRC), dispatches inner entry deserialization, and supports partial buffer processing for streaming reads. It validates entry size against `MAX_ENTRY_SIZE` and delegates inner entry parsing to the appropriate factory.

## 2. Component Specifications (TypeScript Declarations)

```typescript
class GlobalLogEntryFactory {
  static fromU8(buffer: Uint8Array): GlobalLogEntry
  static fromPartialU8(buffer: Uint8Array): { entry?: GlobalLogEntry | LogLogEntry; needBytes?: number; err?: Error }
...
graph TB
    subgraph "GlobalLogEntryFactory"
        ...
        GLEF -->|fromU8| FULL[Full deserialization]
        GLEF -->|fromPartialU8| PART[Partial/streaming]
        GLEF -->|entryLengthFromU8| LEN[Extract length from prefix]
    end

    subgraph "Parsing Steps"
        FULL --> TYPE[Read EntryType byte]
        FULL --> LID[Read 16-byte LogId]
    ...
    subgraph "Validation"
        LEN2 --> MAX[Check MAX_ENTRY_SIZE]
        CRC --> CK[Verify checksum]
    end
```
...
sequenceDiagram
    ...
    participant GF as GlobalLogEntryFactory
    participant Parser as Prefix Parser
    participant Inner as Inner Entry Factory
    participant GLE as GlobalLogEntry

    App->>GF: fromU8(buffer)
    GF->>Parser: extract 27-byte prefix
    Parser-->>GF: {logId, entryNum, entryLength, crc}
    GF->>GF: validate entryType == GLOBAL_LOG
    GF->>Inner: factory for inner entry type
    Inner-->>GF: inner LogEntry
    GF->>GLE: new GlobalLogEntry({...})
    GLE-->>App: GlobalLogEntry instance

    App->>GF: fromPartialU8(buffer)
    alt buffer < 27 bytes
        GF-->>App: { needBytes: 27 - buffer.length }
    else buffer complete
        GF->>Parser: parse prefix, validate
        alt entryLength > MAX_ENTRY_SIZE
            GF-->>App: { err: "Invalid entryLength" }
        else valid
            GF-->>App: { entry: GlobalLogEntry }
        end
    end

    App->>GF: entryLengthFromU8(buffer)
    GF->>Parser: read bytes 21-22 as Uint16LE
    Parser-->>GF: entry length
    GF-->>App: number
```
...
<head>
<meta charset="utf-8">
<title>GlobalLogEntryFactory Animation</title>
<style>
...
(function() {
  ...
  const ANIMATION_KEYFRAMES = [
    { label: "Receive buffer", phase: "input", desc: "Uint8Array arrives for parsing" },
    { label: "Check buffer length", phase: "validate", desc: "At least 27 bytes for prefix?" },
    { label: "Verify EntryType byte", phase: "validate", desc: "First byte must be GLOBAL_LOG (0x00)" },
    { label: "Extract LogId (16B)", phase: "parse", desc: "Bytes 1-16 contain LogId UUID" },
    { label: "Extract entryNum (4B)", phase: "parse", desc: "Bytes 17-20 as Uint32LE" },
    { label: "Extract entryLength (2B)", phase: "parse", desc: "Bytes 21-22 as Uint16LE" },
    { label: "Validate entryLength", phase: "validate", desc: "Must not exceed MAX_ENTRY_SIZE (32768)" },
    { label: "Extract CRC (4B)", phase: "parse", desc: "Bytes 23-26 as Uint32LE" },
    { label: "Deserialize inner entry", phase: "inner", desc: "Parse payload via inner factory" },
    { label: "Return GlobalLogEntry", phase: "output", desc: "Constructed GlobalLogEntry instance" },
  ];
  const ANIMATION_VERIFICATION = [
    "fromU8 throws 'Invalid entryType' if first byte is not GLOBAL_LOG",
    "fromPartialU8 returns needBytes when buffer is too short",
    "entryLengthFromU8 extracts correct length from prefix",
    "fromPartialU8 returns error when entryLength exceeds MAX_ENTRY_SIZE",
    "fromPartialU8 returns needBytes when buffer is shorter than total entry",
    "Deserialized entry matches original entryNum, logId, and cksum",
  ];

  const LEGEND = [
    { label: "Validation", color: "#fb9a99" },
    { label: "Parse", color: "#b2df8a" },
    { label: "Inner Dispatch", color: "#fdbf6f" },
  ...
  svg.append("text")
    ...
    .text("GlobalLogEntryFactory Deserialization");
...
| # | Test | Expected |
|---|------|----------|
| 1 | Deserialize a valid `GlobalLogEntry` from `u8` | `entryNum`, `logId`, inner entry type, and `cksum()` all match original |
| 2 | `fromPartialU8()` with <27 bytes | Returns `{ needBytes: 27 - buffer.length }` |
| 3 | `fromPartialU8()` with `LOG_LOG` type | Delegates to `LogLogEntryFactory` |
...
| 7 | `fromPartialU8()` with entry > `MAX_ENTRY_SIZE` | Returns `{ err }` containing "Invalid entryLength" |
| 8 | `fromPartialU8()` with partial content (< total length) | Returns `{ needBytes }` > 0 |
| 9 | `fromPartialU8()` with valid prefix but invalid inner data | Returns `{ err }` from inner factory |
| 10 | `fromPartialU8()` with valid `LOG_LOG` entry structure | Returns a deserialized entry |
...
| Source Spec | Path |
|---|---|
| GlobalLogEntryFactory.spec.md | `source/src/lib/entry/GlobalLogEntryFactory.spec.md` |
