...
export default class GlobalLogCheckpoint extends LogEntry {
    ...
    constructor({
        ...
        this.lastEntryOffset =
            typeof lastEntryOffset === "number"
                ? new Uint8Array(new Uint32Array([lastEntryOffset]).buffer)
                : lastEntryOffset
    ...
    lastEntryOffsetValue(): number {
        return new Uint32Array(this.lastEntryOffset.buffer)[0]
    }

    lastEntryLengthValue(): number {
        return new Uint32Array(this.lastEntryLength.buffer)[0]
    }
